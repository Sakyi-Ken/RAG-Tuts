...
def metadata_enhanced_search(query, collection, categories=None, min_date=None, top_k=3):
    ...
    new_date = datetime.timestamp(datetime.fromisoformat(min_date)) if min_date else None
    ...
    if categories and new_date:
        where_clause = {"$and": [{"category": {"$in": categories}}, {"date": {"$gt": new_date}}]}
    elif categories:
        where_clause = {"category": {"$in": categories}}
    elif new_date:
        where_clause = {"date": {"gt": new_date}}
    else:
        where_clause = None
    ...
    results = collection.query(
     ...
     # TODO: Check if the initial search returned any documents
    # If no documents were found, perform a fallback search without any filter
...
if __name__ == "__main__":
    ...
    for doc in sample_chunks:
        ...
        metadatas.append({
            ...
            "title": doc["title"],
            "date": datetime.fromisoformat(doc["date"]).timestamp()  # Store date as a timestamp
        })
